...
void RF24::csn(bool mode)
{
	...
	if (ce_pin != csn_pin) {
		RFdigitalWrite(csn_pin,mode);
	} 
	else {
		if (mode == RFHIGH) {
			PORTB |= (1<<PINB2);  	// SCK->CSN RFHIGH
			RFdelayMicroseconds(100); // allow csn to settle.
		} 
		else {
			PORTB &= ~(1<<PINB2);	// SCK->CSN LOW
			RFdelayMicroseconds(11);  // allow csn to settle
		}
	}
...
void RF24::ce(bool level)
{
  ...
  if (ce_pin != csn_pin) RFdigitalWrite(ce_pin,level);
}
...
static const char rf24_pa_dbm_e_str_2[] PROGMEM = "PA_RFHIGH";
...
bool RF24::begin(void)
{
  ...
  uint8_t setup=0;
	...
	RFpinMode(ce_pin,RFOUTPUT);
	ce(LOW);    

	RFdelay(100);
  
  #elif defined(LITTLEWIRE)
    RFpinMode(csn_pin,RFOUTPUT);
    _SPI.begin();
    csn(RFHIGH);
  #else
    // Initialize pins
    if (ce_pin != csn_pin) RFpinMode(ce_pin,RFOUTPUT);  
  ...
  // WARNING: RFdelay is based on P-variant whereby non-P *may* require different timing.
  RFdelay( 5 ) ;
...
{  
  ...
  RFdelayMicroseconds(txRxRFdelay);
  
  if(read_register(FEATURE) & _BV(EN_ACK_PAY)){
    RFdelayMicroseconds(txRxRFdelay); //200
	flush_tx();
  }
  //flush_rx();
  write_register(CONFIG, ( read_register(CONFIG) ) & ~_BV(PRIM_RX) );
...
void RF24::powerUp(void)
{
   ...
   if (!(cfg & _BV(PWR_UP))){
	  ...
	  // There must be a RFdelay of Tpd2stby (see Table 16.) after the nRF24L01+ leaves power down mode before
...
//In this mode, if we can keep the FIFO buffers loaded, packets will transmit immediately (no 130us RFdelay)
//Otherwise we enter Standby-II mode, which is still faster than standby mode
//Also, we remove the need to keep writing the config register over and over and RFdelaying for 150 us each time if sending a stream of data
...
uint8_t RF24::getDynamicPayloadSize(void)
{
  ...
  if(result > 32) { flush_rx(); RFdelay(2); return 0; }
  return result;
}
...
uint8_t RF24::getPALevel(void)
{
  ...
  return (read_register(RF_SETUP) & (_BV(RF_PWR_LOW) | _BV(RF_PWR_RFHIGH))) >> 1 ;
}
...
bool RF24::setDataRate(rf24_datarate_e speed)
{
  ...
  // RFHIGH and LOW '00' is 1Mbs - our default
  setup &= ~(_BV(RF_DR_LOW) | _BV(RF_DR_RFHIGH)) ;
  ...
  {
    // Must set the RF_DR_LOW to 1; RF_DR_RFHIGH (used to be RF_DR) is already 0
    // Making it '10'.
    setup |= _BV( RF_DR_LOW ) ;
  #if defined(__arm__) || defined (RF24_LINUX) || defined (__ARDUINO_X86__)
    txRxRFdelay=450;
  ...
  {
    // Set 2Mbs, RF_DR (RF_DR_RFHIGH) is set 1
    // Making it '01'
    if ( speed == RF24_2MBPS )
    {
      setup |= _BV(RF_DR_RFHIGH);
      #if defined(__arm__) || defined (RF24_LINUX) || defined (__ARDUINO_X86__)
      txRxRFdelay=190;
      #else //16Mhz Arduino	  
	  txRxRFdelay=65;
...
rf24_datarate_e RF24::getDataRate( void )
{
  ...
  uint8_t dr = read_register(RF_SETUP) & (_BV(RF_DR_LOW) | _BV(RF_DR_RFHIGH));
...
/****************************************************************************/
void RF24::setRetries(uint8_t RFdelay, uint8_t count)
{
 write_register(SETUP_RETR,(RFdelay&0xf)<<ARD | (count&0xf)<<ARC);
}
...
void SPIClass::begin() {
  ...
  RFpinMode(USCK, RFOUTPUT);
  RFpinMode(DO, RFOUTPUT);
  RFpinMode(DI, RFINPUT);
  USICR = _BV(USIWM0);
