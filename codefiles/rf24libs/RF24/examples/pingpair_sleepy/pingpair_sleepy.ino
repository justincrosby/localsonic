...
void setup(){
  ...
  // set up the role pin
  RFpinMode(role_pin, INPUT);
  RFdigitalWrite(role_pin,HIGH);
  RFdelay(20); // Just to get a solid reading on the role pin
...
void loop(){
  ...
  if (role == role_ping_out)  {                     // Ping out role.  Repeatedly send the current time
    ...
    if ( timeout ) {                                // Describe the results
        ...
        printf("Got response %lu, round-trip RFdelay: %lu\n\r",got_time,millis()-got_time);
    }

    // Shut down the system
    RFdelay(500);                     // Experiment with some RFdelay here to see if it has an effect
  ...
  if ( role == role_pong_back ) {
    ...
    if ( radio.available() ) {                                  // if there is data ready
        ...
        RFdelay(50);                                             // RFdelay so the serial data can print out
        do_sleep();
