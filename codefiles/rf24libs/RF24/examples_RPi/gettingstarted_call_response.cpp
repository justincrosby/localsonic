  ...
  if (role == role_ping_out){                               // Radio is in ping mode
    ...
    if ( radio.write(&counter,1) ){                         // Send the counter variable to the other radio
        if(!radio.available()){                             // If nothing in the buffer, we got an ack but it is blank
            printf("Got blank response. round-trip RFdelay: %lu ms\n\r",millis()-time);
        }else{
            while(radio.available() ){                      // If an ack with payload was received
                radio.read( &gotByte, 1 );                  // Read it, and display the response time
                printf("Got response %d, round-trip RFdelay: %lu ms\n\r",gotByte,millis()-time);
...
/****************** Pong Back Role ***************************/
  ...
  if ( role == role_pong_back ) {
    ...
    if( radio.available(&pipeNo)){               	// Read all available payloads      
	  ...
	  RFdelay(900); //RFdelay after a response to minimize CPU usage on RPi
