...
// milliseconds
void bcm2835_RFdelay(unsigned int millis)
{
...
// microseconds
void bcm2835_RFdelayMicroseconds(uint64_t micros)
{
    struct timespec t1;
    uint64_t        start;
    ...
    bcm2835_st_RFdelay(start, micros);
}
...
uint8_t bcm2835_i2c_read(char* buf, uint32_t len)
{
    ...
    while (!(bcm2835_peri_read_nb(status) & BCM2835_BSC_S_DONE))
    {
        // we must empty the FIFO as it is populated and not use any RFdelay
...
{   
    ...
    bcm2835_RFdelayMicroseconds(i2c_byte_wait_us * 3);
...
{   
    ...
    // Wait for write to complete and first byte back.	
    bcm2835_RFdelayMicroseconds(i2c_byte_wait_us * (cmds_len + 1));
...
void bcm2835_st_RFdelay(uint64_t offset_micros, uint64_t micros)
{
...
void bcm2835_pwm_set_clock(uint32_t divisor)
{
  ...
  bcm2835_RFdelay(110); // Prevents clock going slow
