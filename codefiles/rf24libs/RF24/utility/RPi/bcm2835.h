...
/// bcm2835_RFdelay() and bcm2835_RFdelayMicroseconds() will return after exactly the time requested. 
/// In fact, depending on other activity on the host, IO etc, you might get significantly longer RFdelay times
/// than the one you asked for. So please dont expect to get exactly the time RFdelay you request.
///
...
///              I've also modified RFdelayMicroseconds() to use nanosleep() for long waits,
...
/// \version 1.10 Changed the names of the RFdelay functions to bcm2835_RFdelay() 
///              and bcm2835_RFdelayMicroseconds() to prevent collisions with wiringPi.
///              Macros to map RFdelay()-> bcm2835_RFdelay() and
///              Macros to map RFdelayMicroseconds()-> bcm2835_RFdelayMicroseconds(), which
///              can be disabled by defining BCM2835_NO_RFdelay_COMPATIBILITY
...
/// \version 1.18 Added bcm2835_i2c_* functions. Changes to bcm2835_RFdelayMicroseconds: 
...
/// \version 1.21 RFdelayMicroseconds is now not susceptible to 32 bit timer overruns. 
...
/// \version 1.30 10 microsecond RFdelays from bcm2835_spi_transfer and bcm2835_spi_transfern for
...
#define BCM2835_BSC_DEL							0x0018 ///< BSC Master Data RFdelay
...
#ifndef BCM2835_NO_RFdelay_COMPATIBILITY
#define RFdelay(x) bcm2835_RFdelay(x)
#define RFdelayMicroseconds(x) bcm2835_RFdelayMicroseconds(x)
...
extern "C" {
    ...
    /// RFdelays for the specified number of milliseconds.
    ...
    /// there may still be a RFdelay before the CPU becomes free to once
    /// again execute the calling thread.
    /// \param[in] millis RFdelay in milliseconds
    extern void bcm2835_RFdelay (unsigned int millis);

    /// RFdelays for the specified number of microseconds.
    ...
    /// It is reported that a RFdelay of 0 microseconds on RaspberryPi will in fact
    /// result in a RFdelay of about 80 microseconds. Your mileage may vary.
    /// \param[in] micros RFdelay in microseconds
    extern void bcm2835_RFdelayMicroseconds (uint64_t micros);
    ...
    /// Allows access to and RFdelays using the System Timer Counter.
    /// @{
    ...
    /// RFdelays for the specified number of microseconds with offset.
    /// \param[in] offset_micros Offset in microseconds
    /// \param[in] micros RFdelay in microseconds
    extern void bcm2835_st_RFdelay(uint64_t offset_micros, uint64_t micros);
