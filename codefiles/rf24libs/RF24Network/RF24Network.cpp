...
void RF24Network::begin(uint8_t _channel, uint16_t _node_address )
{
  ...
  txTimeout = 25;
  routeTimeout = txTimeout*9; // Adjust for max RFdelay per node
...
bool RF24Network::write(RF24NetworkHeader& header,const void* message, uint16_t len, uint16_t writeDirect){
  ...
  while (fragment_id > 0) {
   ...
   header.reserved = fragment_id;
    ...
    if(writeDirect != 070){ RFdelay(2); } //RFdelay 2ms between sending multicast payloads
  ...
  #endif
  networkFlags &= ~FLAG_FAST_FRAG
  //int frag_RFdelay = uint8_t(len/48);
  //RFdelay( rf24_min(len/48,20));
